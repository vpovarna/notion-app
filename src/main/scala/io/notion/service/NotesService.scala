...
final case class NotesService() {
  ...
  def start(): ZIO[DataSource with MongoDatabaseInitializer, Throwable, Unit] = for {
    ...
    _ <- ZIO.service[MongoDatabaseInitializer].flatMap(_.initializer(DBConfig(dbEndpoint, dbPort, dbName)))
